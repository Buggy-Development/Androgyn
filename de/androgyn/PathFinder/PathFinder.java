...
public class PathFinder {
	...
	public PathFinder(int start_x, int start_y, int start_z, int dest_x, int dest_y, int dest_z, int renderDist) {
...
//      printDebugFrame(stack, stack_left.get(0));
	...
	ArrayList<BlockPointer> run(ArrayList<BlockPointer> AvailableList, int start_x, int start_y, int start_z,
		...
		BlockPointer next = get(AvailableList, start_x, /* start_y, */start_z);
		if (next == null)
		...
		while (openList.size() > 0) {
			for (int i = 0; i < openList.size(); i++)
				if (openList.get(i) == null)
					System.out.println("null-entry " + i + " found.");
	...
	BlockPointer getNearest(ArrayList<BlockPointer> openList) {
		...
		float score = best.lastLengthCalc;
		for (BlockPointer b : openList) {
			if (b.lastLengthCalc < score) {
				score = b.lastLengthCalc;
				best = b;
			}
		}
		return best;
